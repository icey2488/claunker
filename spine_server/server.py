...
def _created_by_with_provenance(client_created_by: Any) -> Dict[str, Any]:
    ...
    the board). Unknown non-identity keys pass through with ANY JSON value (interop;
    ``created_by`` is additive-optional and a foreign server may carry a structured
    dialect). The merged shape is re-validated downstream: the Task constructor requires
    the modeled keys (``model``/``effort``/``job_id``) to be strings, and
    ``check_created_by_limits`` bounds the payload size AND nesting depth at the create
    boundary — all surface as ``validation_failed``.
    """
