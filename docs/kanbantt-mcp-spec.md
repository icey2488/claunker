...
- **Unmodeled / foreign fields (preserve-and-round-trip).** Any field OUTSIDE this Card schema — a foreign server's extension, a newer client's not-yet-known field — MUST be preserved and round-tripped (stored intact, echoed on read), never stripped and never an error (see Schema Versioning & Forward Compatibility, and the identical rule applied inside `created_by`). This is the forward-compat guarantee: a v0.9 client's new field survives a round trip through a v0.8 server. A server bounds the preserved payload at the write boundary (the reference implementation reuses the `created_by` interop budget: ≤12 foreign keys, 512 chars per string value, depth 3, 4096 bytes serialized; over → `validation_failed` naming the limit). This is DISTINCT from a KNOWN Card field a server chooses not to model (e.g. `priority`, `checklist`, `attachments` on the reference spine): those keep their documented DEFAULT projection, which a conforming server MUST publish as a divergence rather than present as full support — a documented default is not a silent drop.
...
**Create (`card_create`):**
- `CardInput` is a Card minus the authority-owned fields: no `version`, `created_at`, `updated_at`, `created_by`, `updated_by`, `deleted_at`. Required: `title` (non-empty). Optional with authority defaults (v0.6.0 — the human-intake defaults): `id` (absent → server-minted; supply one to make retries idempotent), `column_id` (absent → the server's intake column: its first Board column, the `created` semantic state on a governance spine), `order` (absent → append at the end of that column). All other fields optional with documented defaults (`priority: "med"`, empty collections, nulls). Authority-owned fields supplied by a client MUST be ignored, not errored (§`created_by`). Fields a server does not model are handled by KIND, never silently dropped (v0.8.0 — resolving the earlier "flatten away" clause that contradicted Design Principle 5 and the Schema-Versioning unknown-field rule): a KNOWN Card field the server does not model (e.g. `priority`/`checklist`/`attachments` on the reference spine) projects its documented DEFAULT, published as a divergence; an UNMODELED / FOREIGN field (outside this Card schema) is PRESERVED and round-tripped (stored intact, echoed back) under the unknown-field rule, bounded at the write boundary (over → `validation_failed`), NOT flattened away. A client value for a genuinely-modeled field is stored; nothing a client sends is silently discarded.
