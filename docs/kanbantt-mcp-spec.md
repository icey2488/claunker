...
`project_list` is the PROJECT-TARGETING read — gated on `canTargetProjects`, derived from that one tool alone, independent of `canWrite` (a server may expose the enumeration read-only, or accept untargeted creates without it). Live projects only, deterministic OLDEST-FIRST order — sorted by `created_at`, ties broken STABLY by creation order (NOT by the random `id`, whose use as a tiebreak made the read non-deterministic on same-tick projects; `created_at` is display-only and never an ordering primitive, so the stable insertion-order tiebreak is what makes the enumeration total and reproducible). Its advertisement is what makes a server **project-aware**: `card_create.project_id` becomes REQUIRED there (see Create semantics), and the client's project picker feeds from this enumeration.
