...
- **`created_by` / `updated_by`** — stamped by the authority on mutation. Servers SHOULD derive actor identity from the authenticated context; agents SHOULD identify with a stable id.
  ...
  - **Unknown-key tolerance, values are strings.** `created_by` MAY carry KEYS a party does not model (a foreign server's provenance dialect). Extra keys MUST be preserved and round-tripped, never stripped, never an error — additive-only, in both directions. But every non-identity VALUE MUST be a string: a server MUST reject a non-string value (a nested object, an array, a number) under ANY key with `validation_failed`. Key tolerance is not value tolerance — this bounds the shape so the payload stays flat and size-boundable.
  - **Admission caps (write-boundary; normative).** Because `created_by` tolerates unknown keys AND is write-once immutable (no cleanup path), a server MUST bound the provenance payload AT CREATE. The reference implementation caps: at most **12** non-identity keys, at most **512** characters per value, and at most **4096 bytes** for the serialized whole object. Over any cap → `validation_failed` naming the specific limit exceeded; the create fails CLOSED (rejected whole, never silently truncated). These numbers are the reference contract; a foreign server MAY choose its own but MUST publish and enforce some finite bound. A legitimate agent mint (`model`+`effort`+`job_id`, ~150 bytes) sits far under all three.
